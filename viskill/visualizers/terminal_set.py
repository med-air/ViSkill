...
from ..modules.sampler import TerminalStateSampler, HierarchicalSampler

WANDB_PROJECT_NAME = 'skill_chaining'
WANDB_ENTITY_NAME = 'thuang22'


class TerminalStateCollector(SkillChainingTrainer):
    def _setup(self):
    ...
    def _setup_sampler(self):
        self.train_sampler = TerminalStateSampler(self.train_env, self.agent, self.env_params)
        self.eval_sampler = TerminalStateSampler(self.eval_env, self.agent, self.env_params)
    ...
    def collect_trajectory(self):
        ...
        for _ in range(self.cfg.n_eval_episodes):
            sc_episode, sl_episode, env_steps = self.eval_sampler.sample_episode(is_train=False, render=True)
            eval_rollout_storage.append(sc_episode)
